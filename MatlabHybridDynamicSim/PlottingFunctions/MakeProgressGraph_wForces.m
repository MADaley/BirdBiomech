function [handle] = MakeProgressGraph_wForces(bodyStates, footStates, time, params, varargin)
%MakeProgressGraph_wForces Graph showing the SLIP model progressing across
%       the screen
%
%   MakeProgressGraph_wForces(bodyStates, footStates, time, params, varargin)
%
% To control the output style, use
%
%   MakeProgressGraph_wForces(bodyStates, footStates, time, params, labelGraph, subPlotData)
%
...
if ~isscalar(subPlotData); %isscalar(subPlotData)
    ...
    %     datamaxY = max(max(subPlotData));
    %     dataminY = min(min(subPlotData));
    datamaxY = 4; % 4*BW
    dataminY =  -2; % -1*BW
